...
function MiniTrend({ values }: { values: number[] }) {
  ...
  return (
    <div className="mt-3 flex h-20 items-end gap-1 rounded-lg border border-[var(--surface-border)] bg-black/10 p-2">
      {values.map((value, idx) => (
        <div
          key={`${value}-${idx}`}
          className="w-full rounded-sm bg-gradient-to-t from-dp/70 to-neural/80"
...
export default function PaperPage() {
  ...
  useEffect(() => {
    const updateActiveSection = () => {
      const anchorY = 170;
      let active = sectionIds[0] ?? 'abstract';
